...
class TNSQuerier:
    ...
    def query_detections(self,
                         ...
                         verbose: bool = True):
        try:
            detection_result = self.query_object(objname = objname, 
    ...
    def search_all(
        self,
        discovery_date_start: str,
        discovery_date_end: str,
        public: str = "all",
        is_tns_at: str = "all",
        classified_sne: int | None = None,
        num_page: int = 100,
        ...
        """
        Query all TNS search records in a date range.
        Dates should be like '2025-01-01' and '2025-12-31'.
        """

        url_parameters = {
            "discovery_date_start": discovery_date_start,
            "discovery_date_end": discovery_date_end,
            "public": public,          # 'yes', 'no', 'all'
            "isTNS_AT": is_tns_at,     # 'yes', 'no', 'all'
            "format": "csv",
            "num_page": str(num_page),
        }

        if classified_sne is not None:
            url_parameters["classified_sne"] = str(int(classified_sne))

        if verbose:
            print(url_parameters)

        file_ = self._search_tns(
        ...
        return ascii.read(file_, format="csv")
    ...
    def _search_tns(self, url_parameters, 
        ...
        url_parameters.setdefault("format", "csv")
        url_parameters.setdefault("num_page", "100")
        ...
        # else, if everything is correct
            ...
            tns_search_file_path = Path(save_dir) / tns_search_file
    ...
    def update_alerthisotry(self, new_alert_path, historyfile_path= None, verbose: bool = True):
        # ?? ??
        new_df = pd.read_csv(new_alert_path)
        
        if historyfile_path == None:
            historyfile_path = self.config.historyfile_path
        try:
...
if __name__ == "__main__":
    ...
    # date_end = "2025-08-13"
    # close_tbl = ascii.read('../utils/close_catalog.ascii_fixed_width', format = 'fixed_width')
    # idx = 2
    # url_parameters = {"ra": str(close_tbl['RA'][idx]), "decl": str(close_tbl['Dec'][idx]), "radius": str(3 * close_tbl['diameter_final'][idx])}
    # file_ = self.search_tns(url_parameters = url_parameters, save_dir = self.config['save_dir'])
    # all_transients = []
# %%
