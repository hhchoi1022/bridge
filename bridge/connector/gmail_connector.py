...
class GmailConnector:
    def __init__(self, 
                 ...
                 user_token_path: str = None):
        self.config = Configuration(config_filenames=['gmailconnector.config'])
        if user_account is None:
            user_account = self.config.user_account
        if user_token_path is None:
            user_token_path = self.config.user_token_path
    ...
    def __repr__(self):
        return f"<GmailConnector(user_account='{self.user_account}', logged_in={self.logged_in_imap or self.logged_in_smtp})>"
    ...
    def login_imap(self):
        ...
        try:
            ...
            self.server_imap = imaplib.IMAP4_SSL(self.imap_server, self.imap_port)
    ...
    def send_mail(self, 
                  to_users: str or list, 
                  subject: str, 
                  body: str, 
                  cc_users : str or list = None, 
                  attachments: list or str = None, 
                  text_type = 'plain'):
        """
        ...
        try:
            ...
            if cc_users is not None:
                cc_users = np.atleast_1d(cc_users)
            to_users = np.atleast_1d(to_users)
    ...
    def read_mail(self, 
        ...
        try:
            ...
            for email_id in email_ids[-max_numbers:]:
                    ...
                    for part in msg.walk():
                        ...
                        if "attachment" in content_disposition:
                            ...
                            if filename:
                                ...
                                # Save the file
                                if save:
                                    save_dir_for_attachments = os.path.join(self.config.save_dir, date_str, 'attachments')
                ...
                if save:
                    save_dir_for_email = os.path.join(self.config.save_dir, date_str)
