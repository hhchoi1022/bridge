...
class SDSConnector:
    def __init__(self):
        self.config = Configuration(config_filenames=['sdsconnector.config'])
        self.tile_path = self.config.tile_path
        self.tbl_RIS = None
    ...
    def find_overlapping_tiles(self, 
        ...
        - list_name: list of names of the coordinates
        ...
        list_aperture = np.array(list_aperture)
        list_aperture = list_aperture / 2

        if list_name is None:
            list_name = [None] * len(list_ra)
        elif len(list_name) != len(list_ra):
            raise ValueError("list_name must have the same length as list_ra and list_dec.")
        ...
        fig_path = None
        if visualize:
            fig_path = self._visualize_tiles(list_ra, list_dec, list_aperture, list_name, list_matched_coords, list_matched_tiles, visualize_ncols, save_fig, visualize_savepath)
    ...
    def _visualize_tiles(self, list_ra, list_dec, list_aperture, list_name, list_matched_coords, list_matched_tiles, visualize_ncols, save_fig, visualize_savepath):
        """
        ...
        list_name = [list_name[i] for i in list_matched_coords]
        ...
        for i, (ra, dec, matched_tile_id, aperture, name) in enumerate(zip(list_ra, list_dec, list_matched_tiles, list_aperture, list_name)):
            ax = axes[i]
            coord_targets = SkyCoord(ra=ra, dec=dec, unit=(u.deg, u.deg))
...
if __name__ == "__main__":
    connector = SDSConnector()
    tbl = connector.find_overlapping_tiles(
        list_ra=[10, 15], 
        list_dec=[-20, -20], 
        list_aperture=3, 
        visualize=True, 
