...
class LSST_Fink_Filter(BaseFilter):
    ...
    def apply_hostgalaxy_filter(self, tbl: Table, return_all: bool = False, verbose=True):
        ...
        for i, row in tqdm(enumerate(tbl), total=len(tbl), desc='Applying host galaxy filter...'):
            ...
            ra = float(row['diaSource_ra'])
            dec = float(row['diaSource_dec'])
    ...
    def apply_magnitude_filter(self, tbl, return_all: bool = False, verbose=True):
        ...
        # --- compute magnitude ---
        flux = tbl['diaSource_psfFlux'].astype(float)
        fluxerr = tbl['diaSource_psfFluxErr'].astype(float)

        tbl['diaSource_psfMag'] = -2.5 * np.log10(flux) + 31.4
        tbl['diaSource_psfMagErr'] = 2.5 / np.log(10) * fluxerr / flux
        tbl['diaSource_psfMagabs'] = tbl['diaSource_psfMag'] - tbl['hostgalaxy_distance_modulus']
