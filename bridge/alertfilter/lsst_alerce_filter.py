...
#%%
# Version: 0.1.0 (2026-02-26, lsst_schema_version = v10_0)

'''
AlertFilter will filter the alerts based on the criteria of the alert. 
It will return True or False depending on the criteria.
Input parameter will be dataframe of the alert(s)

1. Check host galaxy association
2. Check visibility
3. Check duplicated alert
4. Check the alert type
5. Check the alert magnitude and expected magnitude
6. Check the probability of the alert based on the broker
'''
#%%
...
class LSST_Alerce_Filter(BaseFilter):

    def __init__(self, hostgalaxycatalog: HostGalaxyCatalog = None):
        super().__init__('lsst_alerce_filter.config', hostgalaxycatalog)
    ...
    def apply_magnitude_filter(self, tbl, return_all: bool = False, verbose=True):
        ...
        # --- compute magnitude ---
        flux = tbl['psfFlux'].astype(float)
        fluxerr = tbl['psfFluxErr'].astype(float)

        tbl['psfMag'] = -2.5 * np.log10(flux) + 31.4
        tbl['psfMagErr'] = 2.5 / np.log(10) * fluxerr / flux
        tbl['psfMagabs'] = tbl['psfMag'] - tbl['hostgalaxy_distance_modulus']
